...
export class SyncService {
  ...
  private async fetchSyncData(): Promise<SyncDataResponse> {
    try {
      ...
      const response = await fetch(
        `${BASE_URL_API}/public/sync/user/${userId}`,
        {
          method: "GET",
          headers: {
  ...
  private async syncPaiements(paiements: PaiementData[]): Promise<void> {
    ...
    for (const paiement of paiements) {
      await database.runAsync(
        `INSERT OR REPLACE INTO paiements (id, montant, type_paiement, date_paiement, motif, marchand_id,marchandnom, place_id, session_id, agent_id,quittance_id, date_debut, date_fin, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [
          paiement.id,
          paiement.montant,
          ...
          paiement.motif || null,
          paiement.marchandId || null,
          paiement.marchandNom || null,
          paiement.placeId || null,
          paiement.sessionId,
          paiement.agentId,
          paiement.quittanceId || null,
  ...
  public async performSyncManuel(): Promise<{
    success: boolean;
    message: string;
    timestamp?: string;
  }> {
